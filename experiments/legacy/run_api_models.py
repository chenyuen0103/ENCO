...
def _resolve_base_roots(base_root_arg: Path) -> list[Path]:
    ...
    candidate1 = (_EXPERIMENTS_DIR / base_root_arg).resolve()
    candidate2 = (_REPO_ROOT / base_root_arg).resolve()
    candidates = [candidate1, candidate2]

    # Compatibility fallback during migration from prompts/experiment1/<dataset>
    if base_root_arg == Path("prompts"):
        candidates.extend([
            (_EXPERIMENTS_DIR / "prompts" / "experiment1").resolve(),
            (_REPO_ROOT / "experiments" / "prompts" / "experiment1").resolve(),
        ])
...
def main() -> None:
    ...
    if args.provider == "vllm":
        query_script = (_EXPERIMENTS_DIR / "query_vllm.py").resolve()
    else:
        query_script = (_EXPERIMENTS_DIR / "query_api.py").resolve()
