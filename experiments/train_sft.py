...
def _build_reward_eval_callback(
    *,
    python_exe: str,
    sft_output_dir: Path,
    eval_jsonl: Path,
    ...
    eval_script = (Path(__file__).resolve().parent / "eval_cd_rewards_on_jsonl.py").resolve()
    reward_dir = Path(sft_output_dir) / "reward_eval"
    reward_dir.mkdir(parents=True, exist_ok=True)
    summary_path = Path(summary_out) if summary_out is not None else Path(sft_output_dir) / "reward_eval_summary.jsonl"
    summary_path.parent.mkdir(parents=True, exist_ok=True)

    class _RewardEvalCallback(TrainerCallback):
        def __init__(self):
            self._started_steps: set[int] = set()
        ...
        def on_save(self, args, state, control, **kwargs):  # type: ignore[override]
            ...
            if step <= 0:
                return
            if int(every_steps) > 0 and (step % int(every_steps)) != 0:
                return
            if step in self._started_steps:
                return
            self._started_steps.add(step)

            checkpoint_dir = Path(args.output_dir) / f"checkpoint-{step}"
            if not checkpoint_dir.exists():
                print(f"[reward-eval] checkpoint not found at save step {step}: {checkpoint_dir}", file=sys.stderr)
                return

            output_jsonl = reward_dir / f"checkpoint-{step}.jsonl"
            cmd = [
                str(python_exe),
                str(eval_script),
            ...
            if cuda_visible_devices:
                child_env["CUDA_VISIBLE_DEVICES"] = str(cuda_visible_devices)

            print(f"[reward-eval] checkpoint-{step}: running verifier reward eval")
            print("[run]", " ".join(cmd))
            try:
                subprocess.run(cmd, env=child_env, check=True)
                summary = _summarize_grpo_readiness_eval(output_jsonl, checkpoint_dir, Path(sft_output_dir))
                summary.update(
                    {
                        "global_step": step,
                        "epoch": float(state.epoch) if state.epoch is not None else None,
                ...
                print(f"[reward-eval] checkpoint-{step} failed: {exc}", file=sys.stderr)

    return _RewardEvalCallback()
...
def _run_sft_unsloth(
    *,
    ...
    reward_eval_during_sft: bool = False,
    reward_eval_jsonl: Path | None = None,
    reward_eval_n: int = 32,
    reward_eval_num_rollouts: int = 2,
    reward_eval_temperature: float = 0.7,
    reward_eval_top_p: float = 1.0,
    reward_eval_max_new_tokens: int = 1024,
    reward_eval_dtype: str = "auto",
    reward_eval_device_map: str = "auto",
    reward_eval_base_model_override: str | None = None,
    reward_eval_graph_filter: list[str] | None = None,
    reward_eval_cuda_visible_devices: str | None = None,
    reward_eval_every: int = 0,
    reward_eval_summary_out: Path | None = None,
    ...
    if reward_eval_during_sft:
        reward_source_jsonl = reward_eval_jsonl or eval_jsonl or sft_jsonl
        reward_summary = reward_eval_summary_out or (Path(sft_output_dir) / "reward_eval_summary.jsonl")
        print(
            f"[sft/unsloth] verifier reward eval enabled: "
            f"jsonl={Path(reward_source_jsonl).resolve()} summary={reward_summary}"
        )
        if reward_eval_cuda_visible_devices:
            print(f"[sft/unsloth] verifier reward eval CUDA_VISIBLE_DEVICES={reward_eval_cuda_visible_devices}")
        callbacks.append(
            _build_reward_eval_callback(
                python_exe=python_exe,
                sft_output_dir=sft_output_dir,
                eval_jsonl=Path(reward_source_jsonl),
                eval_n=reward_eval_n,
                num_rollouts=reward_eval_num_rollouts,
                temperature=reward_eval_temperature,
                top_p=reward_eval_top_p,
                max_new_tokens=reward_eval_max_new_tokens,
                dtype=reward_eval_dtype,
                device_map=reward_eval_device_map,
                base_model_override=reward_eval_base_model_override,
                graph_filter=reward_eval_graph_filter,
                cuda_visible_devices=reward_eval_cuda_visible_devices,
                every_steps=reward_eval_every,
                summary_out=reward_summary,
            )
        )
    ...
    *,
    ...
    if use_unsloth:
        _run_sft_unsloth(
            ...
            reward_eval_during_sft=reward_eval_during_sft,
            reward_eval_jsonl=reward_eval_jsonl,
            reward_eval_n=reward_eval_n,
            reward_eval_num_rollouts=reward_eval_num_rollouts,
            reward_eval_temperature=reward_eval_temperature,
            reward_eval_top_p=reward_eval_top_p,
            reward_eval_max_new_tokens=reward_eval_max_new_tokens,
            reward_eval_dtype=reward_eval_dtype,
            reward_eval_device_map=reward_eval_device_map,
            reward_eval_base_model_override=reward_eval_base_model_override,
            reward_eval_graph_filter=reward_eval_graph_filter,
            reward_eval_cuda_visible_devices=reward_eval_cuda_visible_devices,
            reward_eval_every=reward_eval_every,
            reward_eval_summary_out=reward_eval_summary_out,
            python_exe=python_exe,
    ...
    if reward_eval_during_sft:
        ...
        reward_summary = reward_eval_summary_out or (Path(sft_output_dir) / "reward_eval_summary.jsonl")
        print(
            f"[sft] verifier reward eval enabled: "
            f"jsonl={Path(reward_source_jsonl).resolve()} summary={reward_summary}"
        )
        if reward_eval_cuda_visible_devices:
            print(f"[sft] verifier reward eval CUDA_VISIBLE_DEVICES={reward_eval_cuda_visible_devices}")
        callbacks.append(
            _build_reward_eval_callback(
                python_exe=python_exe,
                sft_output_dir=sft_output_dir,
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--reward-eval-during-sft",
        action="store_true",
        help="Run verifier reward evaluation on saved SFT checkpoints during training.",
    )
    ap.add_argument(
        "--reward-eval-jsonl",
        type=Path,
        default=None,
        help="JSONL/CSV used for in-training verifier reward eval. Defaults to --eval-jsonl, then --sft-jsonl.",
    )
    ap.add_argument("--reward-eval-n", type=int, default=32)
    ap.add_argument("--reward-eval-num-rollouts", type=int, default=2)
    ap.add_argument("--reward-eval-temperature", type=float, default=0.7)
    ap.add_argument("--reward-eval-top-p", type=float, default=1.0)
    ap.add_argument("--reward-eval-max-new-tokens", type=int, default=1024)
    ap.add_argument("--reward-eval-dtype", choices=["auto", "bf16", "fp16", "fp32"], default="auto")
    ap.add_argument("--reward-eval-device-map", default="auto")
    ap.add_argument(
        "--reward-eval-base-model-override",
        type=str,
        default=None,
        help="Optional full-precision base model/path forwarded to eval_cd_rewards_on_jsonl.py.",
    )
    ap.add_argument("--reward-eval-graph-filter", nargs="*", default=None)
    ap.add_argument(
        "--reward-eval-cuda-visible-devices",
        default=None,
        help="CUDA_VISIBLE_DEVICES for the reward-eval subprocess. Use a GPU not occupied by SFT when possible.",
    )
    ap.add_argument(
        "--reward-eval-every",
        type=int,
        default=0,
        help="Only run reward eval on saved checkpoints whose global step is divisible by this value. 0 means every save.",
    )
    ap.add_argument(
        "--reward-eval-summary-out",
        type=Path,
        default=None,
        help="Path for compact in-training reward eval summaries. Default: <sft_output_dir>/reward_eval_summary.jsonl.",
    )
    ...
    if args.stage in ("all", "sft"):
        ...
        try:
            run_sft(
                ...
                reward_eval_during_sft=args.reward_eval_during_sft,
                reward_eval_jsonl=args.reward_eval_jsonl,
                reward_eval_n=args.reward_eval_n,
                reward_eval_num_rollouts=args.reward_eval_num_rollouts,
                reward_eval_temperature=args.reward_eval_temperature,
                reward_eval_top_p=args.reward_eval_top_p,
                reward_eval_max_new_tokens=args.reward_eval_max_new_tokens,
                reward_eval_dtype=args.reward_eval_dtype,
                reward_eval_device_map=args.reward_eval_device_map,
                reward_eval_base_model_override=(
                    args.reward_eval_base_model_override
                    or args.grpo_readiness_base_model_override
                    or getattr(args, "grpo_base_model", None)
                    or args.distributed_base_model
                ),
                reward_eval_graph_filter=args.reward_eval_graph_filter,
                reward_eval_cuda_visible_devices=args.reward_eval_cuda_visible_devices,
                reward_eval_every=args.reward_eval_every,
                reward_eval_summary_out=args.reward_eval_summary_out,
                python_exe=args.python_exe,
